...
import 'package:deasy_network/src/dialog.dart';
import 'package:deasy_network/src/refresh_token_repository.dart';
      ...
      if (e.response!.statusCode == 401) {
        if (endPoint.path == "/api/v1/auth/refresh-token") {
          DialogUtil.expiredSessionDialog(
            context: context,
            onButtonPress: () {
              DeasyLogout().logout(isExpired: true);
          });
          return null;
        } else {
  ...
  Future<bool> _refreshToken() async {
    try {
      ...
      if (refreshToken != null) {
        final userRefreshToken = await refreshTokenRepository.refreshToken(context, {"refresh_token": refreshToken});

        if (userRefreshToken.data != null) {
          await sharedPreferences.setString("access_token", DeasyEncryptorUtil.encryptString(userRefreshToken.data!.accessToken!));
          await sharedPreferences.setString("refresh_token", DeasyEncryptorUtil.encryptString(userRefreshToken.data!.refreshToken!));
