...
void main() {
  ...
  late OptimusSubmissionSectionController controller;
  ...
  final binding = BindingsBuilder(() {
    ...
    Get.lazyReplace<OptimusAssetRepository>(() => assetRepositoryMock);
  });
    ...
    when(assetRepositoryMock.postApiMasterAsset(any, {}, "")).thenAnswer(
        (_) async => AssetResponse(
            ...
            pageInfo: PageInfo(
                ...
                nextPage: 2)));
    binding.builder();
    controller = Get.find<OptimusSubmissionSectionController>();
    assetData = AssetData(
        assetCode: "tesAssetCode",
        assetName: "tesAssetName",
        assetType: "tesAssetType",
  ...
  tearDownAll(() {
    Get.delete<OptimusSubmissionSectionController>();
  });

  group('Test Submission Section Controller', () {
    test('Test Init Controller', () async {
      expect(() => Get.find<OptimusSubmissionSectionController>(tag: 'success'),
      ...
      expect(Get.isRegistered<OptimusSubmissionSectionController>(), true);
    });
    ...
    test(
      ...
      when(controller.assetRepository.postApiMasterAsset(any, {}, ""))
    ...
    test('phone number validation when phone number value is empty', (() {
      ...
      //act
      //assert
      expect(controller.phoneNumberValidation(""), ValidationConstant.phoneValidation);
    }));
    ...
    test('price good validation when price value is empty', (() {
      ...
      //act
      //assert
      expect(controller.priceGoodValidation(""), ContentConstant.correctPriceGood);
    }));
    ...
    test('price good validation when price value text length is less than 4',
      ...
      //assert
      expect(
          controller.priceGoodValidation("Rp 5"), ContentConstant.correctPriceGood);
    }));
    ...
    test('select item validation when item value not empty', (() {
      ...
      //act
      //assert
      expect(controller.selectItemValidation(""), ValidationConstant.emptyProduct);
    }));
    ...
    test('go to submission section', (() {
      ...
      when(controller.snapAndBuyWebController.stepIndex).thenReturn(RxInt(0));
      ...
      expect(controller.snapAndBuyWebController.stepIndex.value, 0);
